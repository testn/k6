...
```bash
go get go.staging.k6.io/k6
```
This will also build a `k6` binary and put it in `$GOPATH/bin`.

**Building from source**:

Standing in the repo root (`$GOPATH/src/go.staging.k6.io/k6`) you can build a k6 binary from source by running:
```bash
cd $GOPATH/src/go.staging.k6.io/k6
go build
```
...
```bash
go test -race go.staging.k6.io/k6/core
```
...
```bash
go test -race go.staging.k6.io/k6/core -run ^TestEngineRun$
```
